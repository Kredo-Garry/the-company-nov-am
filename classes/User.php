<?php
    ...
    class User extends Database{
        public function store($request){
            #received/request for the datas
    ...
    //             exit();                  //same as die()
    //         }else{
    //             die("Error in createing a user. " . $this->conn->error);
    //         }
    //     }
        ...
        public function login($request){
            ...
            if ($result->num_rows ==  1) { //if true, meaning that the username is available
                ...
                # Check and compare the supplied password against the password already in the databases
                if (password_verify($password, $user['password'])) {
                    # Create the session variables -- to be use later on
                    session_start();
                    $_SESSION['id'] = $user['id'];
        ...
        public function delete(){
            ...
            if ($this->conn->query($sql)) { //if this is okay, call the logout method
                $this->logout(); // call the logou method
            }else {
